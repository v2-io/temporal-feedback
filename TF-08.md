...
$$\rho_B \approx \gamma_A \cdot \mathcal{T}_A \cdot |\Delta a_A| \quad \text{(A's actions change B's world)}$$
$$\rho_A \approx \gamma_B \cdot \mathcal{T}_B \cdot |\Delta a_B| \quad \text{(B's actions change A's world)}$$

where $\gamma_A, \gamma_B > 0$ are the **coupling effectiveness** coefficients — how much of each agent's adaptive output translates into environmental change for the other. These depend on the domain: in direct physical conflict $\gamma$ may be near 1; in information warfare it depends on the target's trust and exposure to the adversary's signals.

**Simplifying assumption.** When coupling is symmetric ($\gamma_A \approx \gamma_B$) and action magnitude is proportional to tempo, the steady-state mismatch ratio simplifies to:
