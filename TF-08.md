...
**Statement.** Under the linear mismatch dynamics hypothesis above, the agent's mismatch remains bounded if and only if $\mathcal{T} > \rho$. More precisely:
...
As $t \to \infty$: $|\delta(t)| \to \rho / \mathcal{T}$, which is finite iff $\mathcal{T} > 0$.

For *bounded* mismatch in the sense that $|\delta|_{ss} < |\delta_{\text{critical}}|$ (some threshold beyond which the model cannot support effective action), the condition becomes $\mathcal{T} > \rho / |\delta_{\text{critical}}|$.

The stronger qualitative claim: when $\rho$ is itself a function of $|\delta|$ (as in the adversarial case where the opponent exploits mismatch), $\rho$ can grow as $|\delta|$ grows, and the condition $\mathcal{T} > \rho$ becomes necessary for stability rather than merely for small steady-state error. $\square$

**Robustness note.** The persistence threshold $\mathcal{T} > \rho$ is *more robust* than the linear dynamics from which it is derived. Any reasonable mismatch dynamics — whether the correction term is linear, concave, or convex in $|\delta|$ — will have a threshold beyond which adaptation cannot keep pace with environmental change. The linear form gives the cleanest expression; the qualitative conclusion holds for any monotone correction function.
...
1. **The mismatch dynamics equation** ($d|\delta|/dt = -\mathcal{T}|\delta| + \rho$) is a first-order linear approximation. The true correction dynamics are almost certainly nonlinear: $d|\delta|/dt = -F(\mathcal{T}, |\delta|) + \rho(t)$. Plausible nonlinearities include:
   ...
   The linear form $F = \mathcal{T} \cdot |\delta|$ is best understood as a local Taylor approximation valid near the steady state. Proposition 8.1's persistence threshold is robust to the specific form of $F$ (see robustness note) because any monotone correction function has a capacity limit. But the *quantitative* predictions (exact steady-state value, convergence rate) are specific to the linear case. A Lyapunov stability analysis (see scratch/) could provide persistence results without committing to a specific $F$.
