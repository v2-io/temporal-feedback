...
$$\mathcal{T} = \nu_{\text{eff}} \cdot \bar{\eta}^* = \sum_k \nu^{(k)} \cdot \eta^{(k)*}$$

This is the effective rate at which the agent reduces model-reality mismatch, combining event rate (how often the agent updates) with update quality (how effectively each update improves the model).
...
From the definition of $\mathcal{T} = \nu \cdot \eta^*$:
