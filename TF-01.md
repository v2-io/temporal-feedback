...
where $\mathcal{H}_t = (o_1, a_1, o_2, a_2, \ldots, a_{t-1}, o_t)$ is the full interaction history (formalized in TF-02 / TF-03). The theory applies wherever there is an agent that observes, acts, and faces residual uncertainty[^shannon1948] about its environment. The final condition — that uncertainty persists even given the complete action-observation history — is constitutive: if the agent could fully determine $\Omega$ from its history alone, the problem would be trivial.
