# TF-01: Scope — Adaptive Agents Under Uncertainty (Axiom)
...
$$\mathcal{S}_{\text{TFT}} = \{(Agent, \Omega) : |\mathcal{O}| > 0, \; |\mathcal{A}| > 0, \; H(\Omega_t \mid o_{1:t}) > 0 \}$$

The theory applies wherever there is an agent that observes, acts, and faces residual uncertainty about its environment. The final condition — that uncertainty persists even after observation — is constitutive: if the agent could fully determine $\Omega$ from observations alone, the problem would be trivial.
...
- **Closed-form systems** — fully observable, fully deterministic ($H(\Omega_t \mid o_{1:t}) = 0$)
