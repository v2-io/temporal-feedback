...
## Proposition 7.1: Structural Adaptation Necessity
...
1. By TF-02, $S(M^*) = \mathcal{F}(\mathcal{M}) < 1 - \epsilon$.
...
4. From TF-04's decomposition, the model error component $|\hat{o}_t - \mathbb{E}[o_t \mid \Omega_t]|^2$ has a positive lower bound that cannot be reduced by any $M \in \mathcal{M}$.
5. The update rule (TF-05) adjusts $M_t$ within $\mathcal{M}$, but by assumption the optimal $M^*$ is already (approximately) reached. Further updates oscillate without net improvement.
...
Proposition 7.1 addresses the case where $\mathcal{M}$ is too constrained — the model class cannot represent the environment's structure. The *opposite* failure is equally important: $\mathcal{M}$ is too expressive, and the model memorizes irreducible noise.
...
2. **Model complexity growing without predictive gain**: In the information bottleneck framing (TF-02), the compression cost $I(M_t; \mathcal{H}_t)$ increases while the predictive power $I(M_t; o_{t+1:\infty} \mid a_{t:\infty})$ plateaus — the model is retaining history detail that carries no predictive value.
...
**Connection to TF-02.** The information bottleneck provides the diagnostic:
...
Structural adaptation is therefore *bidirectional*: expansion when the model class is too constrained (Proposition 7.1), and compression when it is too expressive. The optimal $\mathcal{M}$ sits on the information bottleneck's Pareto frontier for the agent's actual environment.
