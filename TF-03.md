...
A model is **adequate** to the degree that it is a sufficient statistic[^fisher1922] for the history with respect to future prediction. Formally, define **model sufficiency**:
...
The full-sequence $S(M_t)$ remains the theoretical upper bound: $S(M_t) \geq S_\Pi(M_t)$ for any $\Pi$ and any $N_h$. Throughout TFT, results that depend on sufficiency (TF-05's mismatch inevitability, TF-10's structural adaptation trigger) hold for both forms — they require only that the model falls short of perfect sufficiency, which the operational form can detect.
...
[^cover2006]: Cover, T. M. & Thomas, J. A. (2006). *Elements of Information Theory* (2nd ed.). Wiley.
