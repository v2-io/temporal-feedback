...
$$\mathbb{E}[|\delta_t|^2] = \mathbb{E}[|o_t - \hat{o}_t|^2] = \underbrace{\mathbb{E}[|\hat{o}_t - \bar{o}_t|^2]}_{\text{(i) model prediction error}} + \underbrace{\mathbb{E}[\text{Var}(o_t \mid \Omega_t)]}_{\text{(ii) observation noise}}$$
   ...
   (This follows from the law of total variance: $o_t - \hat{o}_t = (\bar{o}_t - \hat{o}_t) + (o_t - \bar{o}_t)$. The cross-term vanishes by the tower property of conditional expectation: $o_t - \bar{o}_t$ is mean-zero given $\Omega_t$, and $\hat{o}_t - \bar{o}_t$ is $\Omega_t$-measurable, so $\mathbb{E}[(o_t - \bar{o}_t)(\hat{o}_t - \bar{o}_t)] = \mathbb{E}[\mathbb{E}[(o_t - \bar{o}_t) \mid \Omega_t] \cdot (\hat{o}_t - \bar{o}_t)] = 0$. This is orthogonality, not independence — the two terms are uncorrelated but need not be independent.)
