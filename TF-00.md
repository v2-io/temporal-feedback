...
| $\delta_t$ | $\in \mathcal{O}$ or $T_M\mathcal{M}$ | Mismatch signal: $o_t - \hat{o}_t$ (primary); or score function $-\nabla_M \log P$ (generalized) | TF-05 |
...
| $g$ | Function | Mismatch transform: maps $\delta_t$ to update direction. Domain/codomain depend on $\delta_t$'s space: $g: \mathcal{O} \to T_M\mathcal{M}$ for prediction errors; $g: T_M\mathcal{M} \to T_M\mathcal{M}$ for score functions | TF-06 |
...
| Prop 9.1: Deliberation Threshold | TF-09 | TF-07, TF-06, Assumption 9.0 | Proved (sketch; conditional on local deliberation drift assumption; see circularity note on $\|\delta_{\text{post}}\|$) |
