...
TF-01 (Scope) → TF-02 (Causal Structure) → TF-03 (Model) → TF-04 (Dynamics)
    ...
    → Appendix A (Lyapunov Stability) → Appendix B (Operationalization)
```
...
- Appendix B → TF-06 + TF-09 + TF-11 + App. A (estimation procedures and worked instantiation)
...
| Prop 9.1: Deliberation Threshold | TF-09 | TF-07, TF-06, Assumption 9.0 | Proved (sketch; conditional on local deliberation drift assumption; see circularity note on $\lvert\delta_{\text{post}}\rvert$) |
