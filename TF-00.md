...
| $S_\Pi(M_t)$ | $\in [0, 1]$ | Model sufficiency — operational form (finite horizon $T_h$, policy class $\Pi$). The measured object. | TF-03 |
...
|----|-----------|-----------------|---------|
| GA-1 | **Fresh noise.** $\varepsilon_t$ is conditionally independent of $\mathcal{H}_{t-1}$ given $(\Omega_t, a_{t-1})$. | TF-01 (implicit), TF-05 (proof of Prop 5.1) | Prop 5.1, mismatch decomposition |
...
| Recursive model update from sufficiency | **Robust qualitative** | Follows from sufficient-statistic property when $S \approx 1$ |
