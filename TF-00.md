...
| $\text{CIY}(a)$ | Scalar (assumed $\geq 0$; see note) | Causal information yield of action $a$. The MI-difference definition can be negative under confounding; non-negativity holds under randomized actions or causal Markov condition (TF-02). Adversarial "negative effective CIY" is discussed in TF-07. | TF-02 |
...
| Action fluency | Qualitative property | Degree to which effective action flows from $M_t$ without deliberation. Distinct from model sufficiency $S(M_t)$. | TF-07 |
| $\lambda(M_t)$ | Scalar $\geq 0$ | Exploration-exploitation balance weight | TF-07 |
...
- TF-05 → TF-01 + TF-03 (Prop 5.1 derives mismatch from scope + model axioms)
- TF-07 → TF-02 (CIY from causal axiom drives exploration)
- TF-09 → TF-06 + TF-11 (deliberation cost depends on gain and tempo)
- TF-10 → TF-03 (structural adaptation changes model class from model axiom)
...
| Prop 9.1: Deliberation Threshold | TF-09 | TF-06, TF-11 | Proved (sketch; see A2 caveat on $\lvert\delta_{\text{post}}\rvert$) |
...
| Unified policy objective ($\pi^* = \arg\max[\text{value} + \lambda \cdot \text{CIY}]$) | TF-07 | Structural analogy with active inference; $\lambda$ not derived |
| Speed-quality substitutability | TF-11 | Follows from $\mathcal{T} = \nu \cdot \eta^*$; qualitative implication |
| Action fluency concept | TF-07 | Qualitative; no formal metric yet |
