...
**Important: hypothesis-dependent vs. robust results.** This document contains two classes of results with different epistemic standing. The *quantitative* results — steady-state mismatch $|\delta|_{ss} = \rho/\mathcal{T}$, speed-quality substitutability, and the adversarial mismatch ratio $|\delta_B|_{ss}/|\delta_A|_{ss} \approx \mathcal{T}_A/\mathcal{T}_B$ — are consequences of the linear ODE hypothesis and should be treated as useful first approximations and design heuristics, not as derived theorems. The *qualitative* results — existence of a persistence threshold, the tempo advantage principle, bounded mismatch under adequate adaptation — are robust to the specific functional form of the correction dynamics and hold under the general nonlinear sector condition proved in [Appendix A](Appendix-A-Lyapunov.md). Hereafter, $\mathcal{T} > \rho$ is used as shorthand for the normalized persistence condition $\mathcal{T} > \rho/|\delta_{\text{critical}}|$ (see TF-00, Units).
...
where $\gamma_A, \gamma_B > 0$ are the **coupling effectiveness** coefficients — how much of each agent's adaptive tempo translates into environmental disruption for the other. These absorb both the coupling strength and the typical action magnitude per adaptive cycle; their value depends on the domain (in direct physical conflict $\gamma$ may be near 1; in information warfare it depends on the target's trust and exposure to the adversary's signals). This is the same formulation used in [Appendix A](Appendix-A-Lyapunov.md), Proposition A.3.

**Simplifying assumption.** When coupling is symmetric ($\gamma_A \approx \gamma_B$) and base disturbance rates are negligible relative to the adversarial coupling, the steady-state mismatch ratio simplifies to:
