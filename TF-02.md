...
The CIY definition above involves a difference of mutual information terms that is not guaranteed to be non-negative in full generality. To use CIY rigorously in downstream equations (the unified policy objective in TF-08, exploration arguments throughout), we distinguish three regimes:

**Regime A — Randomized interventions.** When the agent's actions are randomized (or at least not determined by the same causes that determine the observation), the confounding term $I(o_t; a_{t-1} \mid \Omega_t, M_{t-1})$ vanishes. CIY reduces to $I(o_t; a_{t-1} \mid M_{t-1})$, which is non-negative by construction. This is the standard case for active agents performing exploration (TF-08): RL agents trying different actions, scientists running randomized experiments, organisms probing their environment. **CIY is a well-defined, non-negative, estimable quantity in this regime.**
...
For the purposes of TFT, TF-00 types CIY as $\geq 0$. Downstream equations (particularly TF-08's unified policy objective) assume the agent operates in Regime A or B. The theory's exploration arguments are strongest in Regime A; in Regime B they are conditional on causal identifiability; in Regime C, the CIY term in the policy objective applies to the agent's own probing actions, while adversarial communication is modeled separately as a disturbance.
