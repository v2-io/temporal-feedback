...
Formally, define the **canonical causal information yield** of action $a_{t-1}$:

*[Definition — Canonical Interventional Quantity]*
$$\text{CIY}(a_{t-1}; M_{t-1}) = \mathbb{E}_{a' \sim q(\cdot \mid M_{t-1})}\!\left[D_{\mathrm{KL}}\!\left(P(o_t \mid do(a_{t-1}), M_{t-1}) \,\|\, P(o_t \mid do(a'), M_{t-1})\right)\right]$$

where $q(\cdot \mid M_{t-1})$ is a reference distribution over comparator actions (uniform, policy-induced, or task-specific). This measures how strongly the action changes the interventional distribution of outcomes relative to alternatives.
